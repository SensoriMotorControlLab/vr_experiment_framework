...
public class PinballTask : BilliardsTask
{
  ...
  public override void Setup()
  {
    ...
    else
    {
      ...
      if (ctrler.Session.CurrentTrial.settings.GetString("per_block_hand") == "l")
      {
    ...
    if (!ctrler.Session.CurrentTrial.settings.GetBool("per_block_tilt_after_fire")
        && !dynamicTilt)
      SetTilt();
    ...
    timerIndicator.Timer = ctrler.Session.CurrentTrial.settings.GetFloat("per_block_timerTime");

    timerIndicator.GetComponent<TimerIndicator>().BeginTimer();

    if (ctrler.Session.CurrentTrial.settings.GetString("per_block_indicator_type") == "arc")
    {
    ...
    else if (ctrler.Session.CurrentTrial.settings.GetString("per_block_hand") == "r")
    {
  ...
  private void DynamicForce(float t)
  {
    base.DynamicForce(t, pinball);
  }
